...
case class Options(
    nightlyCheckerCode: Option[String]
)
object Options {
...
object NightlyChecker extends CaseApp[Options] {
  ...
  def run(options: Options, arg: RemainingArgs): Unit = {
    ...
    val code = os.read(os.Path("Hello.scala", os.pwd))
  ...
  private def searchNigthlyStableWorkingVersion(
    ...
    else if (isWorkedScalaVersion(scala3Versions(halfIndex), code)) {
  ...
  private def searchLatestStableWorkingVersion(
    ...
    val (stableVersion, stableIndexVersionIndex) = scala3Versions.zipWithIndex
      .collectFirst {
        case (stable, index) if isWorkedScalaVersion(stable, code) =>
          (stable, index - 1)
      }
      .getOrElse {
